...
class AthenaNoteApp(tk.Tk):
    ...
    def build_menu_frame(self):
        ...
        label = tk.Label(frame, text="Menu", font=("none", 20)).pack()
        create_folder_frame = tk.Frame(frame)
        create_folder_button = tk.Button(create_folder_frame, text="Create Folder", width=12, command=self.create_folder).pack(side="left")
        create_folder_label = tk.Label(create_folder_frame, text="Create a new folder to store '.txt' files.", font=("none", 12)).pack(side='left')
        open_folder_frame = tk.Frame(frame)
        open_folder_button = tk.Button(open_folder_frame, text="Open Folder", width=12, command=self.open_folder).pack(side='left')
        open_folder_label = tk.Label(open_folder_frame, text="Open a folder to edit and add to its existing '.txt' files.", font=("none", 12)).pack(side='left')
